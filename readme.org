#+property: header-args :lexical t :noweb tangle
...
***** Previews
      ...
      and use ~notmuch-show~ to show a candidate. We are also going to
      have a customizable flag for deciding if the preview should
      include the whole thread or just a single matching message:
      ...
      Remember that we've stashed the thread id needed by that
      function as a property of of our candidate string, and provided
      an accessor for it:
