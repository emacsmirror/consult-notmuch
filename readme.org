...
** Embark actions
We can integrate ~consult-notmuch~ with Embark by defining a keymap with actions on notmuch messages and associating it with the completion category of ~notmuch-result~. Here we bind the function to tag search results to ~+~ and ~-~ like in notmuch buffers:
...
~embark-notmuch-tag~ should take as argument the search result as a propertized message string. Because Embark feeds it this string, this function does not need to be interactive:
...
#+BEGIN_SRC emacs-lisp
  (define-key embark-notmuch-map (kbd "d") (embark-notmuch-make-tagger "+trash -inbox"))
  (define-key embark-notmuch-map (kbd "a") (embark-notmuch-make-tagger "-inbox"))
  (define-key embark-notmuch-map (kbd "f") (embark-notmuch-make-tagger "+flagged"))
#+END_SRC

** Embark export

To export search results to a notmuch search buffer with Embark, we can define an exporter:
